...
class RMSCalculator:
	...
	var threshold_count: int = 8  # How many consecutive values need to pass before we tap
	...
	func latest_rms_value() -> float:
		if computed_rms.is_empty():
			return 0.0
		var p: Dictionary = computed_rms[computed_rms.size() - 1]
		if p.has("v"):
			return float(p["v"])
		return 0.0
